...
- **The shortcut is still borrowed** from your session for the run, so the panel prints `Super+Space` rather than the `Super+M` that `just dev` uses. A demo that showed a shortcut nobody has would document a lie.
